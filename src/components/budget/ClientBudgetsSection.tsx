...
import { ChevronRight, TrendingUp, Plus, Users } from "lucide-react"
...
/**
 * A business workspace's per-client spend caps — the v1 budgets list, kept as
 * it was and shown under the spending budgets. Tap a card for its history at
 * /budgets/clients/:key.
 */
export function ClientBudgetsSection() {
  ...
  const label = (b: OverviewBudget) => b.client_name ?? t("budgetsPage.companyDefault")

  const agg = data?.aggregate
  const aggState = agg && agg.total_budget > 0 ? budgetState(agg.total_spent, agg.total_budget).state : "none"

  return (
    <section className="space-y-3" aria-labelledby="client-budgets-title" data-testid="client-budgets">
      <div className="min-w-0">
        <h2 id="client-budgets-title" className="text-base font-semibold tracking-tight flex items-center gap-2">
          <Users className="size-4 text-muted-foreground shrink-0" aria-hidden /> {t("budgets.clientBudgets.title")}
        </h2>
        <p className="text-xs text-muted-foreground mt-0.5">{t("budgets.clientBudgets.subtitle")}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed py-8 text-center">
          ...
          {canWrite && (
            <Button variant="outline" size="sm" className="mt-3" onClick={() => setSetDefaultOpen(true)}>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300">
            {data.budgets.map((b) => (
              <button
                key={b.key}
                type="button"
                onClick={() => navigate(`/budgets/clients/${b.key}`)}
      ...
      <BudgetDialog
        ...
        clientId={null}
        label={t("budgetsPage.companyDefault")}
