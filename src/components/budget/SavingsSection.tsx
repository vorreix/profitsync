      ...
      <Card className="py-0">
        <CardContent className="p-4">
          <p className="text-sm font-semibold">
            {t("budgetV2.sectionSavings")}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            {t("budgetV2.savingsEmpty")}
          </p>
          <Button
            size="sm"
            variant="outline"
            ...
            return (
              <li key={env.id}>
                <div className="flex items-start gap-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="flex items-center gap-1.5 truncate text-xs font-medium">
                        ...
                        {spaceBacked && (
                          <span className="inline-flex items-center gap-0.5 rounded bg-muted px-1 text-[10px] font-normal text-muted-foreground">
                            <ArrowRightLeft className="size-2.5" aria-hidden />{" "}
                            {t("budgetV2.spaceBacked")}
                          </span>
                        )}
                      </p>
                      ...
                      <p className="text-[11px] text-muted-foreground tabular-nums">
                        {env.funding_mode === "virtual"
                          ? t("budgetV2.fundBalance", {
                              amount: money(env.balance ?? 0),
                            })
                          : spaceBacked
                            ? t("budgetV2.fundInSpace")
                            : t("budgetV2.fundHeldBack")}
                        {env.goal_amount != null &&
                          ` / ${money(env.goal_amount)}`}
                      </p>
                      ...
                      >
                        {state === "confirmed"
                          ? t("budgetV2.contributionConfirmed", {
                              amount: money(env.planned),
                            })
                          : state === "missed"
                            ? t("budgetV2.savingsMissed")
                            : state === "skipped"
                              ? t("budgetV2.contributionSkipped")
                              : t("budgetV2.contributionPlanned", {
                                  amount: money(env.planned),
                                })}
                      </p>
                      ...
                      {env.goal_progress && (
                        <div className="mt-1.5">
                          <div
                            ...
                            aria-valuemin={0}
                            aria-valuemax={100}
                            aria-label={t("budgetV2.goalProgress", {
                              name: env.name,
                            })}
                          >
                            <div
                              ...
                              style={{
                                width: `${Math.min(100, Math.max(0, env.goal_progress.pct))}%`,
                              }}
                            />
                          </div>
                          <p className="mt-1 text-[11px] text-muted-foreground tabular-nums">
                            {env.goal_progress.reached
                              ...
                              : t("budgetV2.goalRemaining", {
                                  amount: money(env.goal_progress.remaining),
                                })}
                  ...
                  {/* A fund is editable and deletable like any other envelope —
                    same dialog, same rules. Without this, savings were the one
                    section you could create into but never change. */}
                  {canWrite && (
                    <button
                      type="button"
                      onClick={() => onEdit(env)}
                      aria-label={t("budgetV2.editEnvelope", {
                        name: env.name,
                      })}
                      className="pressable mt-1 flex size-9 shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
                    >
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-1 px-1">
                    ...
                    {state === "planned" && spaceBacked && (
                      <p className="text-[11px] text-muted-foreground">
                        {t("budgetV2.spaceTransferRequired")}
                      </p>
                    )}
                    {state === "planned" && (
                      <Button
