...
| `/api/budgets/v2` | Budget v2 plan: GET (the ONE aggregate read — read-only, reports `sync_required`), POST (the wizard's single write; `spending_target` required), PATCH (settings + pause/resume, owner/admin for settings/status), DELETE (archive). Personal workspaces only |
...
| `/api/budgets/v2/envelopes` (+ `/:id`, `/:id/detail`, `/reorder`) | envelopes: create (one category → one envelope, 409 names the owner; a Space must be this org's) · rename/retarget/recategorise (a cadence-only change re-plans; the catch-all cannot be removed) · drill-through (same inclusion predicates as the totals) · drag order |
