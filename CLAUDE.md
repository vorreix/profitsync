...
| `SpendingBudget` | `spending_budgets` | **Budgets (v3)** — a named limit over a window (`period`: daily\|weekly\|monthly\|yearly\|once + optional dates), scoped to expense `categories` (jsonb; `[]` = all spending), with one level of sub-budgets (`parent_id`; a child's window is its PARENT's, resolved on read, never stored). Spend is never stored — summed live by `api/_lib/spending-budgets.ts`. `status` active\|paused. Both workspace types. See `docs/budget/BUDGETS.md` |
...
| `/api/spending-budgets` · `/api/spending-budgets/:id` · `/api/spending-budgets/reorder` | **Budgets (v3)**: list with live spend + per-period section summaries (GET materialises due recurring rows → `ALWAYS_FETCH`), create; detail (sub-budgets, spend-vs-limit series against the limit in effect at each window's close, recent rows, audit history), PATCH, DELETE (cascades sub-budgets); sibling order. `docs/budget/BUDGETS.md` |
...
- **Budgets** (`/budgets`, both workspace types): named spending limits over a window (daily/weekly/monthly/yearly or custom dates), scoped to expense categories or to all spending, with one level of **sub-budgets** ("Household €2,000 → Groceries €600, Transport €200, the rest unclaimed"). Spend is never stored: one aggregate statement sums the ledger per budget (standard outgoing + refunds negative, never transfers, never system rows, never trashed or closed clients) on the same category key `lower(btrim())` everywhere. A section header never sums rows (overlapping scopes would double-count) — it shows the union spend and a limit only when scopes are disjoint. Pure math `src/lib/budget.ts`, SQL `api/_lib/spending-budgets.ts`. A **business** workspace additionally keeps its per-client spend caps (`budgets` + `budget_history`, `/budgets/clients/:key`). Full rules: `docs/budget/BUDGETS.md`.
