...
- Migrations are in `drizzle/` and run automatically on `vercel-build` (`scripts/db-migrate.mjs`). Current head is **0069** (0060–0068 are hand-written; 0059–0061, the retired Budget v2 engine, are no longer in the journal — production never ran them — `drizzle-kit generate` is out of sync with them; write the SQL + journal entry by hand). **Journal gotcha:** the migrator keeps ONE watermark per database (`max(created_at)`) and walks the journal in ARRAY order, so a new entry stamped at or below it is skipped **silently** while `db:migrate` prints "up to date". `npm run migrations:check` (`scripts/check-migrations.mjs`, in the hook and CI) enforces: unique numbers, journal and folder agreeing, `when`/`idx`/filename all strictly increasing, nothing future-dated, and every migration from 0069 on individually re-runnable. Numbering **gaps are fine** — 0059–0061 are the retired Budget v2 migrations. Full model: the `migrations` skill.
...
| `/api/debts` (+ `/:id`, `/:id/payments`, `/:id/payments/:paymentId`) | Debt & Loans hub payload / create · detail + schedule, edit terms & lifecycle, reconcile (system Balance Adjustment), close · record a repayment (principal transfer + interest/fee expenses in ONE ledger group) · trash a payment group. Engine: `api/_lib/debts.ts`; pure math in `src/lib/debt-*.ts` |
