...
- Migrations are in `drizzle/` and run automatically on `vercel-build` (`scripts/db-migrate.mjs`). Current head is **0061** (0059–0061 are Budget v2; 0060/0061 are hand-written — write the SQL + journal entry by hand). **Journal gotcha:** a new migration can silently skip ("up to date" but column missing) when `drizzle/meta/_journal.json` `when` values were normalized — bump the new entry's `when` above the previous, then verify the column exists in `information_schema`.
- **Local development uses the Neon database in `.env.local`** — there is no local Postgres/docker setup. `npm run db:migrate` and the seed/backfill scripts load `.env.local` via dotenv (non-overriding — an exported `DATABASE_URL` still wins, and on Vercel the file doesn't exist so the platform env is used). A script that imports `src/lib/db` (client built at import time, e.g. `scripts/migrate-budgets-v2.ts`) must be run with `npx tsx --env-file=.env.local …`. Migrations are additive; apply pending ones with `npm run db:migrate`. **Never run `npm run db:push` against a shared database** — it diffs the live schema and will propose dropping columns that exist there from unmerged branches.
