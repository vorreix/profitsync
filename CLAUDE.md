...
|---|---|
| `budget-v2` | **Any** budgeting work — the `/budgets` screen, plans, periods, envelopes, commitments/bills, occurrences, savings funds, reallocation, overspend resolution, refunds/settlements, period close/restatement, the v1→v2 migration, or touching the `budget_*`/`transaction_settlements` tables or `GET /api/budgets`. Establishes the three-layer engine (pure math → DB engine → routes) and the 12 money invariants — invoke it **first**. |
...
npm run dev          # Full-stack on :5173 — vite.config.ts localApiPlugin serves /api/* in-process (same dispatch as api/index.ts), env from .env.local
...
| `Budget` / `BudgetHistory` | `budgets`, `budget_history` | **v1** per-client/own-company spend caps + adherence/creep history (keyed by org+client so it survives "remove"). Still the surface for BUSINESS workspaces; for a personal workspace with a v2 plan, `GET /api/budgets`/`overview`/`detail` serve a projection of the plan (`api/_lib/budget-v1-adapter.ts`) — the v1 shape is kept indefinitely for store-pinned native bundles |
| `BudgetPlan` (v2) | `budget_plans` | ONE live plan per **personal** org (partial unique index); cadence `monthly|weekly|payday|custom` (+ anchors; a custom cadence must carry `custom_start` — CHECK), `income_mode` `expected|available`, `included_account_ids` (validated: this org's live bank/cash ids), `status` `active|paused|archived` |
| `BudgetEnvelope` (v2) | `budget_envelopes` | a durable line item in a `section` `flexible|commitment|debt|savings|income`; `match_keys` (normalised category keys — one category → one envelope), the catch-all (`is_catch_all`, cannot be removed), `target_amount`+`target_cadence`, carry policy/cap, savings `funding_mode` `virtual|space_backed` (+ `wealth_account_id` → a Space, FK NO ACTION), soft-removed (`status`). **The budgets list** (`docs/budget-v2/SIMPLE.md`, mig 0063): `kind` `category|group` (a group is a macro budget = the SUM of its children, no target/keys of its own), `parent_id` (one level, catch-all never grouped), `hidden` (display only — still counts); `status='paused'` = **inactive** (counts nowhere, spend falls back to the catch-all) |
| `BudgetPeriod` / `BudgetAllocation` (v2) | `budget_periods`, `budget_allocations` | at most one OPEN period per plan; each envelope's `planned_amount` + `rollover_in` per period (rollover is written IN the allocation insert, never separately); funding base + source recorded at open |
| `BudgetCommitment` / `BudgetOccurrence` (v2) | `budget_commitments`, `budget_occurrences` | a bill/debt payment (one-time or bound to a `recurring_rules` row) and its DEVIATIONS only (settled/cancelled/skipped/rescheduled) — occurrences are projected, an occurrence is an expectation, settling never moves money |
| `BudgetPeriodSnapshot` / `BudgetEvent` (v2) | `budget_period_snapshots`, `budget_events` | the frozen report of a closed period (versioned; restatement appends, never edits; carries a drift `__fingerprint`) · the append-only audit trail every mutation writes in the same batch |
| `BudgetFundEntry` / `BudgetExclusion` / `TransactionSettlement` (v2) | `budget_fund_entries`, `budget_exclusions`, `transaction_settlements` | confirmed fund contributions (balance = SUM) · "not a refund" opt-outs · an inflow explicitly settling an expense (Σ per expense ≤ expense, Σ per inflow ≤ inflow, enforced on write AND clamped on read) |
...
- Migrations are in `drizzle/` and run automatically on `vercel-build` (`scripts/db-migrate.mjs`). Current head is **0063** (0059–0063 are Budget v2; 0060–0063 are hand-written — write the SQL + journal entry by hand, `when` strictly above the previous). **Journal gotcha:** a new migration can silently skip ("up to date" but column missing) when `drizzle/meta/_journal.json` `when` values were normalized — bump the new entry's `when` above the previous, then verify the column exists in `information_schema`.
- **Local development uses the Neon database in `.env.local`** — there is no local Postgres/docker setup. `npm run db:migrate` and the seed/backfill scripts load `.env.local` via dotenv (non-overriding — an exported `DATABASE_URL` still wins, and on Vercel the file doesn't exist so the platform env is used). A script that imports `src/lib/db` (client built at import time, e.g. `scripts/migrate-budgets-v2.ts`) must be run with `npx tsx --env-file=.env.local …`. The Budget v2 migrations (0059–0063) are additive; apply pending ones with `npm run db:migrate` (it prints the target host first). **Never run `npm run db:push` against a shared database** — it diffs the live schema and will propose dropping columns that exist there from unmerged branches.
...
| `/api/budgets` · `/api/budgets/overview` · `/api/budgets/detail` | v1 budgets + adherence/creep (business workspaces); for a personal workspace with a v2 plan these serve the plan's PROJECTION (`api/_lib/budget-v1-adapter.ts`) — the v1 shape is kept indefinitely |
| `/api/budgets/v2` | Budget v2 plan: GET (the ONE aggregate read — read-only, reports `sync_required`; `?window=week\|month\|year` re-windows ONLY the `budgets` list block, the four numbers stay on the period), POST (the wizard's single write; `spending_target` required), PATCH (settings + pause/resume, owner/admin for settings/status), DELETE (archive). Personal workspaces only |
| `/api/budgets/v2/sync` | the ONLY writer of budget state: reconcile settlements, close every elapsed period (walking the chain, capped + resumable), open the next one WITH its rollover, refresh the base, restate drifted closed periods |
| `/api/budgets/v2/envelopes` (+ `/:id`, `/:id/detail`, `/reorder`) | envelopes: create (one category → one envelope, 409 names the owner; a Space must be this org's; `kind:"group"` creates a macro budget, `parent_id` puts a category inside one) · rename/retarget/recategorise/`parent_id`/`hidden`/`status active\|paused` (a cadence-only change re-plans; the catch-all cannot be removed, grouped or paused; removing a group detaches its children) · drill-through (same inclusion predicates as the totals) · order |
| `/api/budgets/v2/commitments` (+ `/:id`) · `/api/budgets/v2/occurrences` | bills/debt payments · act on ONE projected occurrence (settle/cancel/skip/reschedule — a deviation row, never a transaction; reschedule collisions and undo handled at write time) |
| `/api/budgets/v2/reallocate` · `/refunds` · `/contributions` · `/prompts` | move planned money (ONE atomic CAS statement; Σplanned invariant) · link/unlink/reject refunds (race-safe caps) · confirm/skip a savings contribution (reserved-neutral) · resolve migration prompts |
...
- **Budgets** — two generations on ONE route. A **business** workspace keeps **v1** per-client / own-company spend caps (`budgets` + `budget_history`, adherence + creep) at `/budgets`. A **personal** workspace gets **Budget v2** (`docs/budget/BUDGETS.md`, `docs/budget-v2/SMART_HYBRID_BUDGET_SPEC.md`, the `budget-v2` skill): a household plan with periods, envelopes (flexible / bills / debt / savings), commitments and projected occurrences, refunds/settlements, reallocation and overspend resolution, period close with rollover and versioned snapshots. Four numbers answer "can I spend this?" — `available_now`, `reserved`, `safe_to_spend` (+ which limit bound), `forecast_balance`. Reads never write; `POST /api/budgets/v2/sync` is the only writer. Business workspaces get NO plan (their `budgets` rows ARE their caps). The v1→v2 migration is `scripts/migrate-budgets-v2.ts` (additive, idempotent; a lifetime cap becomes a paused plan awaiting the user's choice).
