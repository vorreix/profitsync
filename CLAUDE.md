...
- Migrations are in `drizzle/` and run automatically on `vercel-build` (`scripts/db-migrate.mjs`). Current head is **0068**; everything from **0059** on is hand-written (`drizzle-kit generate` is out of sync with them, so write the SQL and the journal entry by hand). The numbering is **contiguous** — the retired Budget v2 migrations were pulled and the ones after them renumbered to close the hole. **Journal gotcha:** the migrator keeps ONE watermark per database (`max(created_at)`) and walks the journal in ARRAY order, so an entry stamped at or below it is skipped **silently** while `db:migrate` prints "up to date". Stamp `when` with `Date.now()`, append the entry LAST, and never edit an existing `when` — it is the only field compared, so changing it re-applies or skips. `npm run migrations:check` (`scripts/check-migrations.mjs`, in the hook and both CI workflows) enforces all of it. Full model: the `migrations` skill.
