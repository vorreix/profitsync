...
| `budgets` | **Any** budgets work — `/budgets`, spending budgets and sub-budgets, category scope, the `spending_budgets` table, `/api/spending-budgets`, the v1 per-client caps at `/api/budgets`, budget notifications. Establishes the *spend is never stored / one aggregate statement / child window derived on read* model and its invariants — invoke it **before** touching the math. |
| `migrations` | **Any** database migration — adding one, renumbering, a merge conflict in `drizzle/meta/_journal.json`, or the symptom "the column is missing but `db:migrate` says up to date". Establishes the ONE-WATERMARK model: drizzle walks the journal in array order against a single `max(created_at)` per database, so anything out of order is skipped **silently**. |
...
| `i18n` | **Any** user-visible string — a new label, button, toast, error, placeholder, aria-label or empty state — or a locale file, `src/lib/i18n/`, a red i18n gate, or a symptom like "this page is still in English in Malayalam". Establishes the rule the gate rests on: **a key that EXISTS is not a key that is TRANSLATED**, and a value copied from English now fails the check. |
...
A husky **pre-commit hook** (`.husky/pre-commit`, installed via the `prepare` script on `npm install`) gates every commit with: secret scan → `check-esm-extensions` → `boot-functions` → `check-route-guards` → `check-migrations` → `cache:check` → `i18n:check` → `lint` → `typecheck` → `test:ci`. TypeScript (`tsc --noEmit`) and ESLint are the primary static analysis tools. Test coverage is partial — unit tests live in `src/lib/*.test.ts`.
...
AI_CREDITS_FREE_GRANT=500                 # AI credit economy: free = ONE-TIME grant, premium = monthly refill
AI_MONTHLY_CREDITS_PREMIUM=10000          #   (no rollover; downgrade caps leftover at the free grant)
AI_CREDITS_QUICKADD=5                     # base costs (text / receipt+voice fill / assistant ask) + token
AI_CREDITS_QUICKADD_MEDIA=10              #   surcharge beyond included budgets — see api/_lib/ai.ts +
AI_CREDITS_ASSISTANT=20                   #   src/lib/ai-credits.ts (pure math, tested)
AI_TOKENS_INCLUDED_QUICKADD=3000
AI_TOKENS_INCLUDED_ASSISTANT=6000
AI_TOKENS_PER_EXTRA_CREDIT=1000
```
...
| App | `/dashboard`, `/clients`, `/clients/closed`, `/clients/:id`, `/clients/:id/files`, `/transactions`, `/recurring`, `/recurring/:id`, `/calendar`, `/flow`, `/wealth` (Banks; `?tab=cards` = Cards), `/wealth/cards/:cardId`, `/wealth/:id`, `/analytics`, `/categories`, `/budgets` (`?tab=analytics` = Analytics), `/budgets/:id`, `/budgets/clients/:key`, `/referrals`, `/quotations`, `/organizations`, `/organizations/:id/members`, `/subscription`, `/trash`, `/profile`, `/onboarding`, `/organization-setup` | `AppLayout` |
...
| `WealthAccount` | `wealth_accounts` | `organization_id`, `type`: `cash\|bank\|space\|credit_card`, `opening_balance`, `current_balance` (SIGNED asset-equivalent — a credit card's is negative = amount owed), `credit_limit`/`statement_closing_day`/`payment_due_day` (cards), `is_default`, `archived_at`; `Cash` auto-provisioned + permanent. Liability math lives in `src/lib/credit-card.ts` — never read the sign in a component. **Colour identity** (mig 0075, presentation only): `color` (`''` = AUTO, else `#RRGGBB`) + `color_style` (`subtle\|bold`) — resolved in ONE place, `src/lib/account-color.ts` (user's hex → the bank's curated brand colour, the same table the cards use → a stable per-row swatch; Cash/Spaces have fixed defaults). Never read the columns in a component — call `accountAppearance()` |
| `Debt` | `debt_details` + `debt_payments` | a debt IS a `wealth_accounts` row (`type`: `loan` = I owe, balance negative; `receivable` = owed to me, positive) plus its TERMS in `debt_details` (kind, counterparty, native `currency`, rate, scheduled payment + frequency, next due, lifecycle `active\|paused\|paid_off\|refinanced\|written_off`). `debt_payments` = the principal/interest/fees allocation of one recorded repayment, anchored on its ledger group (live while the anchor leg isn't trashed). Borrowing = transfer debt→bank (never income); principal = transfer bank→debt (never expense); interest/fees = expenses on the paying account. Everything else (status, progress, obligations, debt-free date, planner) is DERIVED — `src/lib/debt-math.ts`, `debt-status.ts`, `debt-planner.ts`. `kind` is FREE TEXT with nine suggestions. A debt may be serviced by a recurring repayment (`recurring_rules.kind='debt'`), created in the SAME atomic batch as the debt; that rule is then the single source of truth for the schedule and `payment_amount`/`payment_frequency`/`next_due_date` MIRROR it. See `docs/debts/DEBTS.md` |
...
| `Card` | `cards` | a DEBIT or CREDIT card linked to a bank — **identity + attribution only, never money**: `account_id` = the ledger account it posts to (a debit card's bank; a credit card's liability account, 1:1), `funding_account_id` (credit: the account the statement is paid FROM) + `funding_card_id` (the optional instrument on that side, always resolving to `funding_account_id` — a debit card means the money still leaves its bank; a credit card is a BALANCE TRANSFER, so **autopay is refused on a liability funder** on both write paths and again in the engine, which is also what makes a funding cycle impossible), `issuer_account_id` (credit: the bank that GAVE you the card — a real account, picked or created in wizard step 1, so an issuer counts against the plan's bank limit; NULL on cards predating mig 0062, which fall back to the liability account's `bank_name`), `last4` (only the last four digits are ever stored), network/expiry/holder/`tier`/`design`/`brand_colors`, `autopay` (+ `autopay_since`, opt-in), `status`: `active\|frozen\|closed` (derived `closed` when the account is archived). `transactions.card_id` / `recurring_rules.card_id` = which card paid; the server forces `wealth_account_id = card.account_id` on every write (`api/_lib/cards.ts attributeCard`). Autopay engine: `api/_lib/card-autopay.ts`. See `docs/cards/CARDS.md` |
| `RecurringRule` | `recurring_rules` | `organization_id`, anchor + frequency; lazily **materializes** due transactions on GETs (no cron). Tx carry `recurring_rule_id` (+ `card_id` when the rule pays with a card; rules pause while the card is frozen/closed). `kind`: `standard` \| `transfer` (a Space auto-save) \| **`debt`** (a repayment: `debt_account_id` is the debt, `wealth_account_id` the account it is paid FROM; principal posts as a transfer and interest/fees as expenses, the split recomputed from the live balance each occurrence, the last instalment capped at the payoff figure — `api/_lib/recurring-debt.ts`). An EXISTING rule can be adopted as a debt's repayment from either screen (`PATCH /api/recurring/:id { debt_account_id }`, forward only — what already posted is never rewritten); one repayment per debt, enforced by a partial unique index (mig 0068) |
| `Budget` / `BudgetHistory` | `budgets`, `budget_history` | **business** per-client spend caps + adherence/creep history (keyed by org+client so it survives "remove"). A personal workspace has NO row here since mig 0064 — its v1 budget became its first spending budget, and `GET/POST /api/budgets` project that budget for store-pinned bundles |
| `SpendingBudget` | `spending_budgets` | **Budgets (v3)** — a named limit in a rhythm (`period`: daily\|weekly\|monthly\|yearly\|once + optional dates), scoped to expense `categories` (jsonb; `[]` = all spending), with one level of sub-budgets (`parent_id`; a child's window is its PARENT's, resolved on read, never stored). The **overall budget** is the top-level row with `categories = []` — exactly ONE per workspace, a partial unique index on `(organization_id) where parent_id is null and categories = '[]' and status = 'active'` (mig 0065) + 409 `overall_exists`; siblings are disjoint at every level. Spend is never stored — summed live by `api/_lib/spending-budgets.ts`, for the budget's own window AND all four view windows. `status` active\|closed (`paused` renamed in 0065). Both workspace types. See `docs/budget/BUDGETS.md` |
...
- Migrations are in `drizzle/` and run automatically on `vercel-build` (`scripts/db-migrate.mjs`). Current head is **0075**; everything from **0059** on is hand-written (`drizzle-kit generate` is out of sync with them, so write the SQL and the journal entry by hand). The numbering is **contiguous** with ONE reserved hole: **0069–0074 belong to the unmerged `feat/multi-currency` branch**, whose migrations are already applied to the shared dev database — so a new migration numbers ABOVE them rather than taking one back, because two different migrations on one number is the failure the guard exists to stop, while a gap only costs a reader a moment. That reservation lives in `scripts/check-migrations.mjs`; **delete it the day that branch merges**. Retiring a migration no database ever ran is legitimate (the Budget v2 trio was pulled that way) and the ones after it are renumbered to close the hole. **Journal gotcha:** the migrator keeps ONE watermark per database (`max(created_at)`) and walks the journal in ARRAY order, so an entry stamped at or below it is skipped **silently** while `db:migrate` prints "up to date". Stamp `when` with `Date.now()`, append the entry LAST, and never edit an existing `when` — it is the only field compared, so changing it re-applies or skips. `npm run migrations:check` (`scripts/check-migrations.mjs`, in the hook and both CI workflows) enforces all of it. Full model: the `migrations` skill.
- **No local database.** All local work — `npm run db:migrate`, throwaway checks, Playwright — runs against the Neon `DATABASE_URL` in `.env.local` (the migrator loads it itself). The instance is shared and carries columns from unmerged branches, so migrations must stay **additive** and you must **never run `npm run db:push`** against it — it diffs the live schema and will propose dropping what other branches added.
...
| `/api/wealth/transfer` | account-to-account transfer (`kind=transfer`; excluded from income/expense). **Paying a credit card is this** with the card as `to_account_id` — never an expense. `from_card_id` names the card used on the source side: a debit card is an instrument, a credit card is a balance transfer |
| `/api/debts` (+ `/:id`, `/:id/payments`, `/:id/payments/:paymentId`) | Debt & Loans hub payload (materialises due repayments first → `ALWAYS_FETCH`) / create the debt **and its repayment in one atomic `dbBatch`** — a new rule via `repayment`, or one that already exists via `link_rule_id` (mutually exclusive) · detail: `debt` + `activity` (every money event on the debt, one row per ledger group) + `payments` + `schedule` + `repayment`; edit terms, lifecycle (which pauses/resumes the rule), the repayment itself, reconcile (system Balance Adjustment), close · record a repayment (principal transfer + interest/fee expenses in ONE ledger group; with a live rule a hand-recorded payment is EXTRA and does not move the due date) · trash a payment group. Engine: `api/_lib/debts.ts` + `recurring-debt.ts`; pure math in `src/lib/debt-*.ts` |
| `/api/wealth/bank-search` · `/api/wealth/quota` | bank logo/name autocomplete · free-plan bank gating |
| `/api/recurring` · `/api/recurring/:id` | recurring rules (materialize lazily on GETs); accept `card_id` (the rule pays with a card) and `debt_account_id` (POST creates the rule already linked, writing the debt's mirrored schedule in the same batch; PATCH links/unlinks and must arrive ALONE). GET `:id` is one rule for its own page `/recurring/:id` — the list's joined fields plus what it has posted (`generated_count`, `posted_total`, first/last posted date); PATCH answers in the same shape. Its payments are `GET /api/transactions?recurringRuleId=` (flat, transfers + closed clients included, so the list matches the count) |
| `/api/cards` · `/api/cards/reorder` (drag order; one atomic UPDATE) · `/api/cards/:id` · `/api/cards/:id/summary` | **Wealth & Cards**: debit/credit cards linked to banks (GET runs the card sync: statements → autopay → alerts; POST creates the bank inline via `new_bank` and, for credit, the liability account; PATCH identity/status/funding/autopay/credit config — close with debt → 409 `card_has_debt`; summary = credit view + `next_autopay`, or debit month activity). `GET /api/transactions?cardId=` lists a card's rows (flat, transfers included); every tx write accepts `card_id`; `/api/wealth/transfer` accepts `from_card_id`; `/api/search` returns a `cards` group. `docs/cards/CARDS.md` |
| `/api/alerts` | **the dashboard attention rail** — what needs the user now (overdue/soon card payments, autopay outlook, expiring cards, a charge the account won't cover, paused recurring rules, money that just landed). Derived FRESH from current state, never from the notifications log, and strictly READ-ONLY (it must never materialize money). `docs/alerts/ALERTS.md` |
...
| `/api/spending-budgets` · `/api/spending-budgets/:id` · `/api/spending-budgets/reorder` · `/api/spending-budgets/analytics` | **Budgets (v3)**: list with live spend for the budget's own window **and all four view windows** (`spent_by_view`, so the page's Day/Week/Month/Year toggle costs no request; GET materialises due recurring rows → `ALWAYS_FETCH`), create; detail (sub-budgets, spend-vs-limit series against the limit in effect at each window's close, recent rows, audit history), PATCH (closing a parent cascades to its children), DELETE (cascades sub-budgets); sibling order (one atomic UPDATE); analytics (`?view=&back=`, read-only, 3 round trips — trend vs the limit that applied then, unclaimed spend, adherence over finished+reliable windows, per-budget utilisation). `docs/budget/BUDGETS.md` |
| `/api/budgets` · `/api/budgets/overview` · `/api/budgets/detail` | business per-client spend caps + adherence/creep; on a PERSONAL org these project the primary spending budget in the v1 shape (store-pinned bundles) |
...
`en.json` is the source of truth for which keys must exist. **Two gates, both in the pre-commit hook and in `.github/workflows/i18n.yml`:** `npm run i18n:check` fails when a key is missing, blank, has lost an interpolation `{{placeholder}}`, **or is still the English text** — a key that exists is not a key that is translated, and the old presence-only check passed for months while ~470 keys sat in English in every language and `/debts` rendered half in English for anyone reading Malayalam. `npm run i18n:hardcoded` fails on NEW user-visible English typed straight into JSX, which never reaches a locale file at all; the existing backlog is frozen per file in `src/lib/i18n/hardcoded-baseline.json` and may only go down. Strings that genuinely ARE identical in another language (a brand, `IBAN`, sample data, or a loanword like `Status` in German) are listed per locale in `src/lib/i18n/identical-ok.json` with a reason. The `_one`/`_zero`/`_two` plural variants may omit `{{count}}` (idiomatic in some languages). For bulk backfills, `scripts/i18n-merge.mjs` deep-merges a `{ lang: { "dotted.key": value } }` map into the locale files (additive by default, `--overwrite` to correct existing values). Full recipe: the `i18n` skill.
...
- **Budgets** (`/budgets`, both workspace types): named spending limits in a rhythm (daily/weekly/monthly/yearly or custom dates), scoped to expense categories or to all spending, with one level of **sub-budgets** that split a parent BY CATEGORY. The page has two tabs (**Budgets** | **Analytics**, `?tab=`) and one **view toggle** (Day/Week/Month/Year): it reports on ONE window and converts every budget into it — conversions pivot through a DAY (`PERIOD_DAYS`/`perDayRate`, so a week is exactly 7 days), and a window actually being judged uses `limitForWindow`, pro-rated by its REAL day count (a 28-day February allows 28 days of a daily rate, not the mean month's 30.44). The **overall budget** (top level, no categories, one per workspace) is the header, and the others are measured against it individually and as a sum (`allocation`) — honest only because **siblings are disjoint at every level** (the overall budget, custom-date and closed budgets exempt). Sub-budgets are managed inline on every row; **Reorder** is a mode with two independent single-level drag scopes; **closed** budgets fold away and closing a parent cascades to its children. Spend is never stored: one aggregate statement sums the ledger per (window, scope) pair (standard outgoing + refunds negative, never transfers, never system rows, never trashed or closed clients) on the same category key `lower(btrim())` everywhere. Pure math `src/lib/budget.ts`, SQL `api/_lib/spending-budgets.ts`. A **business** workspace additionally keeps its per-client spend caps (`budgets` + `budget_history`, `/budgets/clients/:key`). Full rules: `docs/budget/BUDGETS.md`.
...
- **Debt & Loans** (`/debts`, `wealth_accounts.type='loan'|'receivable'` + `debt_details` + `debt_payments`): a debt is a first-class liability account (a receivable an asset). Quick (who / how much / what you pay / when) or detailed setup; payments split into principal (transfer) and interest/fees (expenses); statuses derived; estimated debt-free date only when it can be computed; the planner (`src/lib/debt-planner.ts`) compares minimum / avalanche / snowball / free-up-cash / custom order with the rollover engine and switches to stabilisation when scheduled payments exceed the budget. Debt accounts are hidden from `GET /api/wealth/accounts` (like Spaces) and rejected by the plain transaction endpoints **and by `createTransfer`** — money reaching a loan must go through the debt engine, which splits principal from interest. Global search returns them as their own group, never as bank accounts. A debt can be serviced by a **recurring repayment**, and either half can make the other in ONE request — the debt screen creates or adopts a rule, the recurring screen creates or picks a debt; eligibility is one predicate (`src/lib/debt-recurring.ts` `linkRefusal`) run client-side to decide what is offered and server-side to decide what is accepted — the client must fill in EVERY field the server does (an archived payer, an expired rule, instalments still waiting) or it offers what the save refuses — with **one repayment per debt** enforced by a partial unique index (mig 0068) and a settled debt (paid off / refinanced / written off) refused outright while a PAUSED one is accepted and simply deactivates the rule; the rhythm travels as the rule's own (unit, interval) and the debt's `payment_frequency` is derived from it (`irregular` when it has no name) — naming the nearest one turned "every 2 years" into "monthly";  creating one also asks how much of the money actually landed in an account (**partial is normal** — the part that arrived is a transfer, the rest a system Opening Balance, and the two always sum to what is owed), and a **live preview** (`src/lib/debt-preview.ts`, pure) shows the payoff date, instalment count, interest and total as they type, or refuses to invent a date when the payment cannot cover the interest. Full design: `docs/debts/DEBTS.md`.
...
- **Attention rail** (`/dashboard` only): a swipeable, severity-coloured carousel of what needs the user right now — an overdue card payment, a charge tomorrow the account can't cover, a card expiring, a recurring payment that has silently stopped, the salary that just landed. Every item is **derived fresh from current state, never read from the notifications table**: a log row keeps saying "overdue" after the card is paid, so the rail disappears the moment the thing stops being true (the bell is history; the rail is now). Pure model in `src/lib/alerts.ts`, SQL in `api/_lib/alerts.ts`, both fully tested. Full invariants — the as-of-today baseline, the unclamped credit-card floor, when autopay may be blamed — in `docs/alerts/ALERTS.md`.
