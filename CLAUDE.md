...
| `/api/debts` (+ `/:id`, `/:id/payments`, `/:id/payments/:paymentId`) | Debt & Loans hub payload (materialises due repayments first → `ALWAYS_FETCH`) / create the debt **and its optional recurring repayment in one atomic `dbBatch`** · detail: `debt` + `activity` (every money event on the debt, one row per ledger group) + `payments` + `schedule` + `repayment`; edit terms, lifecycle (which pauses/resumes the rule), the repayment itself, reconcile (system Balance Adjustment), close · record a repayment (principal transfer + interest/fee expenses in ONE ledger group; with a live rule a hand-recorded payment is EXTRA and does not move the due date) · trash a payment group. Engine: `api/_lib/debts.ts` + `recurring-debt.ts`; pure math in `src/lib/debt-*.ts` |
| `/api/wealth/bank-search` · `/api/wealth/quota` | bank logo/name autocomplete · free-plan bank gating |
| `/api/recurring` · `/api/recurring/:id` | recurring rules (materialize lazily on GETs); accept `card_id` (the rule pays with a card). GET `:id` is one rule for its own page `/recurring/:id` — the list's joined fields plus what it has posted (`generated_count`, `posted_total`, first/last posted date); PATCH answers in the same shape. Its payments are `GET /api/transactions?recurringRuleId=` (flat, transfers + closed clients included, so the list matches the count) |
