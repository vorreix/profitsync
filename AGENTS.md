...
- Migrations are in `drizzle/` and run automatically on `vercel-build` (`scripts/db-migrate.mjs`). Current head is **0068** (0060–0068 are hand-written; 0059–0061, the retired Budget v2 engine, are no longer in the journal — production never ran them — `drizzle-kit generate` is out of sync with them; write the SQL + journal entry by hand). **Journal gotcha:** a new migration can silently skip ("up to date" but column missing) when `drizzle/meta/_journal.json` `when` values were normalized — bump the new entry's `when` above the previous, then verify the column exists in `information_schema`.
...
- **`npm run cache:check`** (pre-commit + `pr.yml`, and a `PostToolUse` hook in `.Codex/settings.json`) re-derives the side-effecting GET routes from the API source and every write path from `src/`, and fails when the map disagrees.
