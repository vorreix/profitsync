...
## 11. Checkout currency & payment connectors (the India card fix)

**Symptom.** Customers in some regions (notably **India**) hit **"Missing connector
response"** when paying by **card** on the hosted checkout.

**Cause.** Dodo routes each charge to a payment **connector** chosen by the
**{billing country × transaction currency}** pair. Our products are **USD-priced**, and
`create-subscription` sent only `billing.country` (from `x-vercel-ip-country`) — **no
`billing_currency`**. So an Indian customer was charged in **USD with `country = IN`**, a
pair that has **no eligible Indian-card connector** → "Missing connector response". Dodo's
own docs: *"For India-specific payments, the billing country must be set to `IN` with
currency `INR`."*

**Fix (code).** `create-subscription.ts` now:
- resolves the billing **country** from the user's **profile country** first (authoritative
  for billing), then the IP geo, then `US`;
- derives **`billing_currency`** from that country via `currencyForCountry` (IN → INR,
  every mapping is a Dodo-supported ISO currency) and passes it to `createSubscription`;
- seeds the **full billing address** (state/city/street/zipcode) from the profile.

This makes Dodo bill the customer in their **local currency**, routing to a connector that
can process their card (and surfacing local methods like **UPI** for IN+INR — we do **not**
set `allowed_payment_method_types`, since that is a *whitelist* that would hide valid
methods like iDEAL/SEPA elsewhere). `change-plan` is unaffected — it reuses the
already-attached payment method (no hosted card entry), so the connector error can't occur
there.

**Required Dodo dashboard config (operational — not code).** The code fix only helps if the
account can actually settle these methods:
- Enable the **India card connector** (and/or **adaptive/local currency**) for the
  merchant account, and enable **UPI** if you want it offered.
- Indian-card **subscriptions** require **RBI-compliant e-mandates** (Dodo handles the
  mandate; note the ~48h processing delay for Indian cards/UPI).
- Keep the relevant currencies enabled in **Settings → Payment Methods**.

**Test-mode caveat.** In test mode the **test card must match the billing country**:
- `IN` billing → use an **INR test card** (Visa `4576238912771450`, MC `5409162669381034`)
- `US` billing → use a **US test card** (Visa `4242424242424242`)

Using a US test card against an `IN` billing country reproduces the same connector/decline
error in test mode — it's the card↔country mismatch, not the integration.
