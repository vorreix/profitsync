...
| 08 | `feat/cattags-08-tag-quota` | Per-plan tag ceiling per transaction (free 1 / premium 3): server 402 backstop + up-front upgrade chip | ✅ done |

Order rationale: schema first, then the backend it enables (tags before categories because tags
need the migration), then the shell, then the two UI tabs, then the plan gate. Each branch passes the gate before push.
...
- **2026-07-11** — Branch 08 shipped: **per-plan tag ceiling on a single transaction** (free = **1**, premium = **3**). Single source of truth `src/lib/tags.ts` (`FREE_TAGS_PER_TX`/`PREMIUM_TAGS_PER_TX`/`tagLimitForPlan(isPaid)`); surfaced in the plan model as `plans.limits.tagsPerTransaction` (added to `PlanLimits` + `DEFAULT_FREE_LIMITS`/`DEFAULT_PREMIUM_LIMITS` + the `getOrgPlan` merge in `api/_lib/quota.ts`, so it's editable per-plan later and enforced by default even for un-seeded plans). New `checkTransactionTagQuota(orgId, tagCount, {previousCount})` returns the standard `QuotaCheck`; **server backstop** returns HTTP 402 on the three tag-mutating routes — `transactions.ts` (POST), `transactions/group.ts` (split POST), `transactions/[id].ts` (PATCH) — always **dedup/normalize first** (`cleanTransactionTags`) so duplicates can't bypass, and PATCH passes `previousCount` so **legacy over-limit rows stay editable** (only *increasing* past the cap is refused — grandfathering the tags that shipped in #273). **Up-front UX** (`TagsInput`): at the cap the text input is swapped for an amber **Crown "Upgrade for more tags"** chip (existing tags stay removable so a user can always reduce) + a "You've reached your plan's tag limit." hint; clicking it closes the dialog and `navigate("/subscription")`. Client reads the ceiling from `tagLimitForPlan(isPaidPlanKey(activeOrg?.plan_key))` (no extra fetch) and threads `tagLimit`/`onTagUpgrade` through `TxFormFields` from both the Add dialog and the edit dialog on `TransactionsPage`; `apiErrorUpgradeHint(err)` maps a server 402 to the same toast + redirect as a safety net. New i18n `transactions.tagsUpgrade` + `transactions.tagsLimitReached` across all 8 locales (1456 keys in sync). **Verified with Playwright** on the Free test org: adding `#coffee` (1 tag) swapped the input to the amber upgrade chip + hint, the chip navigated to `/subscription`, and an end-to-end create with 1 tag succeeded (no regression in the `group.ts` path); server enforcement + grandfathering proven via a throwaway DB test (free limit = 1; 2 tags → 402 `upgradeHint:true`; legacy 2-tag row still PATCH-able), test data cleaned up. Gate green.
